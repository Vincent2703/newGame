module("libraries.shadowsStar", package.seeall)

Shadows		=		require("shadows")
Light		=		require("libraries.shadows.Light")
Transform	=		require("libraries.shadows.Transform")
...
function Star:Update()
	...
	if self.Changed or self.World.Changed or self.World.UpdateStars then
		...
		love.graphics.setShader(libraries.shadowsShapeShader)
		...
		if self.Image then
			...
			libraries.shadows.LightShader:send("Radius", self.Radius * wz)
			libraries.shadows.LightShader:send("Center", { self:GetCanvasCenter() })
		...
		if self.Blur then
			...
			love.graphics.origin()
			love.graphics.setShader(libraries.shadowsRadialBlurShader)
			libraries.shadowsRadialBlurShader:send("Size", { self.Canvas:getDimensions() })
			libraries.shadowsRadialBlurShader:send("Position", { self:GetCanvasCenter() })
			libraries.shadowsRadialBlurShader:send("Radius", self.Radius * wz)
